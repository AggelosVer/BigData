...
\title{\textbf{Επεξεργασία Δεδομένων Ροής με Kafka, Spark και MongoDB}\\
\large Συστήματα Διαχείρισης Μεγάλων Δεδομένων (CEID\_NE4348)}
\author{
...
\begin{center}
    \includegraphics[width=0.65\textwidth]{producer_output.png} 
    \captionof{figure}{Εκτέλεση του παραγωγού και αποστολή μηνυμάτων.}
    
    \vspace{0.5cm} % Μικρό κενό ανάμεσα στις εικόνες
    
    \includegraphics[width=0.65\textwidth]{redpanda_topic.png} 
...
\begin{minipage}[t]{0.45\textwidth}
  Η πιο κρίσιμη διεργασία λαμβάνει χώρα εντός του Apache Spark. Όπως παρατηρούμε στο Σχήμα 3, το Spark χωρίζει τη συνεχή ροή σε διακριτά "Batches". 
  
  Στον πίνακα της κονσόλας φαίνονται ξεκάθαρα οι στήλες που έχουμε ορίσει: ο χρόνος προσομοίωσης (\texttt{time}), το τμήμα του δικτύου (\texttt{link}) και οι βασικές μετρικές, δηλαδή το πλήθος των οχημάτων (\texttt{vcount}) και η μέση ταχύτητά τους (\texttt{vspeed}).
...
Αμέσως κάτω από τους πίνακες, το Spark επιβεβαιώνει την επικοινωνία του με τη βάση δεδομένων εκτυπώνοντας τα μηνύματα \texttt{MongoDB OK}. Σημειώνεται ότι εκτός από τα \texttt{[stats]} και \texttt{[raw\_data]}, το σύστημα αποθηκεύει επιτυχώς και τα \texttt{[windowed\_stats]}, τα οποία αφορούν τον υπολογισμό των κινητών μέσων όρων ανά 30 δευτερόλεπτα (το δεύτερο Bonus).
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\textwidth]{mongo_collections.png} 
    \caption{Ανάκτηση εγγραφών (raw και aggregated) από τη MongoDB.}
\end{figure}

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{aggregation.png} 
    \caption{Εκτέλεση του Aggregation Query για την εξαγωγή του συνολικού μέσου όρου.}
\end{figure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.7\textwidth]{rest_api.png} 
    \caption{Εκκίνηση του REST API Server.}
\end{figure}
